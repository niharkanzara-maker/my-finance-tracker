...
//  With Supabase Auth + Cloud Database
// ═══════════════════════════════════════════════
...
async function signUp() {
  ...
  if (profErr) { setMsg(msgEl,'err','Account created but profile failed: ' + profErr.message); return; }

  setMsg(msgEl,'ok','Account created! Please check your email to verify, then log in.');
}

async function generateUniqueId() {
  const { data } = await supabase.from('profiles').select('unique_id').order('created_at', { ascending: false }).limit(1);
...
async function loadProfile() {
  ...
  if (error || !data) {
    // profile not found — ask for name
    showPage('pg-home');
    showPanel('signup');
    return;
  }
...
async function getRules() {
  const { data } = await supabase.from('rules').select('*').eq('user_id', currentUser.id);
  return data || [];
}

async function saveRulesToDB(rules) {
  // delete existing rules and re-insert
  await supabase.from('rules').delete().eq('user_id', currentUser.id);
...
function toggleAddForm() {
  $('add-txn-form').classList.toggle('hide');
}
...
async function renderSummary() {
  ...
  if (ctx) {
    charts['cat-donut'] = new Chart(ctx, {
      type:'doughnut',
      data:{ labels:Object.keys(CAT_MAP), datasets:[{ data:total>0?catData:[1,1,1,1], backgroundColor:Object.values(CAT_COLORS), borderWidth:0, hoverOffset:5 }] },
      options:{ responsive:true, maintainAspectRatio:false, cutout:'65%', plugins:{ legend:{display:false}, tooltip:{callbacks:{label:c=>total>0?' '+fmt(c.parsed):' No data'}} } }
    });
  }
}
...
async function renderAnnual() {
  ...
  const annSubTotals = {};
  allTxns.forEach(t => {
    const sk = (t.category||'Other')+'|'+(t.subcategory||'Other');
    annSubTotals[sk] = (annSubTotals[sk]||0) + parseFloat(t.amount);
  });
  ...
  ${annExpSubs.length?`<div class="section"><div class="sec-title">Annual expenses breakdown</div>
    ${annExpSubs.map(s=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:.45rem 0;border-bottom:.5px solid #1e2130">
      <span>${s.name}</span><div style="text-align:right"><div style="font-size:13px;font-weight:500">${fmt(s.val)}</div>
      <div style="font-size:11px;color:#8b90a8">${sumExp>0?Math.round(s.val/sumExp*100):0}% of expenses</div></div></div>`).join('')}
  </div>`:''}
  <div class="section">
...
async function renderRules() {
  ...
  <div class="section">
    ...
    <p style="font-size:12px;color:#8b90a8;margin-bottom:1rem;line-height:1.7">
      Keywords matched against bank statement descriptions for auto-categorization.
    </p>
    <div class="field" style="max-width:240px;margin-bottom:1rem">
      ...
      <select id="rules-bank">
        ...
        <option value="${b}" ${(currentProfile.bank||'kotak')===b?'selected':''}>
...
async function saveRules() {
  ...
  // update bank in profile
  await supabase.from('profiles').update({ bank: $('rules-bank').value }).eq('id', currentUser.id);
...
function renderUpload() {
  ...
  <div class="section">
    ...
    <p style="font-size:12px;color:#8b90a8;margin-bottom:1rem;line-height:1.7">
      Upload your Kotak Mahindra bank statement PDF.
    </p>
  ...
  <div id="confirm-section" class="hide">
    <div class="section">
      ...
      <p style="font-size:12px;color:#8b90a8;margin-bottom:.85rem">
        <span style="color:#F0997B">■</span> Orange rows = unmatched — please select a category.
      </p>
      <div class="tbl-wrap">
        <table class="data-tbl">
...
function parseKotakPDF(items, rules) {
  ...
  const colWdRight = colX.dep - 5;
  const colDepLeft = colX.dep - 5;
  const colDepRight= colX.bal - 5;
  ...
  lines.forEach(line => {
    const dateItem = line.items.find(it =>
      dateRe.test(it.str.trim()) && it.x >= colX.date-30 && it.x <= colX.date+80
    );
    if (dateItem) {
      ...
      const descItems = line.items.filter(it =>
        inDesc(it.x) && !isAmt(it.str) && !dateRe.test(it.str.trim()) && !srRe.test(it.str.trim())
      );
      ...
      const contItems = line.items.filter(it =>
        ...
        !isAmt(it.str) && !dateRe.test(it.str.trim()) && it.str.trim().length>1
      );
      if (contItems.length) {
        const extra = contItems.map(i=>i.str).join(' ').trim();
        if (extra && !/^\d+$/.test(extra)) current.desc += ' ' + extra;
      }
    }
  });
  if (current && current.desc.length>1 && current.amount>0) transactions.push(current);
    ...
    .map(t => {
      ...
      for (const r of rules) {
        if (r.keyword && fuzzyMatch(t.desc, r.keyword)) {
          cat=r.category; subcat=r.subcategory; break;
        }
      }
