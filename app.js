...
window.showPanel = function(t) {
  ...
  if (t==='signup') {
    cards.style.display='none'; signup.style.display='block'; login.style.display='none';
  } else if (t==='login') {
    cards.style.display='none'; signup.style.display='none'; login.style.display='block';
  } else {
    cards.style.display='block'; signup.style.display='none'; login.style.display='none';
  }
}
...
window.logIn = async function() {
  ...
  try {
    ...
    let msg = e.message;
    if (e.code==='auth/invalid-credential')  msg = 'Incorrect email or password. Please try again.';
...
window.goHome  = window.logOut;
window.openDemo = function() { showPage('pg-demo'); }
...
async function saveRulesToDB(rules) {
  ...
  await sb.from('rules').insert(rules.map(r=>({user_id:currentUser.id,keyword:r.keyword,category:r.cat,subcategory:r.subcat})));
}
...
function calcNetWorthFromData(openingBals, allTxns) {
  ...
  allTxns.forEach(t=>{
    ...
    if(t.category==='Assets')      txnAssets      += t.type==='Withdrawal'?amt:-amt;
    if(t.category==='Liabilities') txnLiabilities += t.type==='Deposit'?amt:-amt;
  });
  const totalAssets      = openingAssets+txnAssets;
  const totalLiabilities = openingLiabilities+txnLiabilities;
  return { openingAssets, openingLiabilities, txnAssets, txnLiabilities, totalAssets, totalLiabilities, netWorth:totalAssets-totalLiabilities };
}
...
window.toggleAddForm=function(){const f=$('add-txn-form');f.style.display=f.style.display==='none'?'block':'none';}
window.updateSubDrop=function(subId,catId){const cat=$(catId).value,sub=$(subId);sub.innerHTML='<option value="">Select sub-category</option>'+(CAT_MAP[cat]||[]).map(s=>`<option>${s}</option>`).join('');}

window.saveTxnRow=async function(){
  const date=$('new-date').value,desc=$('new-desc').value.trim();
  const amount=fmtN($('new-amt').value),type=$('new-type').value;
  const cat=$('new-cat').value,subcat=$('new-subcat').value;
  ...
  const{m,y}=getMonthYear();
  await sb.from('transactions').insert({user_id:currentUser.id,date,description:desc,amount,type,category:cat,subcategory:subcat,month:m,year:y});
  renderTxn();
}
...
async function renderSummary(){
  ...
  const txns=await getTxns();
  if(!txns.length){body.innerHTML=`<div class="section"><div class="empty"><div class="empty-icon">📊</div>No transactions for ${MFULL[m]} ${y}.</div></div>`;return;}
  let income=0,expenses=0,assetsIn=0,assetsOut=0,liabIn=0,liabOut=0;
  const expSubs={},incSubs={},astSubs={};
  txns.forEach(t=>{
    const amt=parseFloat(t.amount),sub=t.subcategory||'Other';
  ...
  const netAssetChange=assetsIn-assetsOut,netLiabChange=liabIn-liabOut;
  const surplus=Math.max(0,income-expenses-netAssetChange);
  const totalSavings=income-expenses,savingsRate=income>0?Math.round(totalSavings/income*100):0;
  ...
  const sugFlexi=totalSavings*.6,sugLiquid=totalSavings*.3,sugIns=totalSavings*.2;
  body.innerHTML=`
  <div class="section">
    ...
    <div class="grid3" style="margin-bottom:1.25rem">
      ...
      <div class="metric"><div class="lbl">Total savings</div><div class="val" style="color:#4d9fff">${fmt(totalSavings)}</div><div class="sub">Savings rate: ${savingsRate}%</div></div>
    </div>
    <div style="display:flex;justify-content:space-between;font-size:12px;color:#8892b0;margin-bottom:4px"><span>Savings rate</span><span style="color:#00d4a0;font-weight:600">${savingsRate}%</span></div>
  ...
  <div class="section">
    ...
    <div style="margin-top:1rem">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding:.5rem 0;border-bottom:1px solid rgba(99,120,220,0.08)">
        <div style="display:flex;align-items:center;gap:8px"><span style="width:10px;height:10px;border-radius:3px;background:${color};display:inline-block"></span><span style="font-size:13px">${label}</span></div>
        <div style="text-align:right"><div style="font-size:13px;font-weight:600;color:${color}">${fmt(val)}</div><div style="font-size:11px;color:#8892b0">${income>0?Math.round(val/income*100):0}% of income</div></div>
      </div>`).join('')}
    </div>
  </div>
  ${expSubArr.length?`<div class="section"><div class="sec-title">Expenses breakdown</div>${expSubArr.map(([name,val])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:.5rem 0;border-bottom:1px solid rgba(99,120,220,0.08)"><span style="font-size:13px">${name}</span><div style="text-align:right"><div style="font-size:13px;font-weight:600">${fmt(val)}</div><div style="font-size:11px;color:#8892b0">${expenses>0?Math.round(val/expenses*100):0}%</div></div></div>`).join('')}</div>`:''}
  ${incSubArr.length?`<div class="section"><div class="sec-title">Income breakdown</div>${incSubArr.map(([name,val])=>`<div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid rgba(99,120,220,0.08)"><span style="font-size:13px">${name}</span><span style="font-size:13px;font-weight:600;color:#00d4a0">${fmt(val)}</span></div>`).join('')}</div>`:''}
  ${astSubArr.length?`<div class="section"><div class="sec-title">Investments this month</div>
    ...
    ${astSubArr.map(([name,val])=>`<div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid rgba(99,120,220,0.08)"><span style="font-size:13px">${name}</span><span style="font-size:13px;font-weight:600;color:#4d9fff">${fmt(val)}</span></div>`).join('')}
  </div>`:''}
  ${liabIn>0||liabOut>0?`<div class="section"><div class="sec-title">Liabilities this month</div>
    <div class="grid3"><div class="metric"><div class="lbl">Loans taken</div><div class="val" style="color:#a78bfa">${fmt(liabIn)}</div></div><div class="metric"><div class="lbl">Repaid</div><div class="val" style="color:#00d4a0">${fmt(liabOut)}</div></div><div class="metric"><div class="lbl">Net change</div><div class="val" style="color:${netLiabChange<=0?'#00d4a0':'#ff6b6b'}">${netLiabChange>0?'+':''}${fmt(netLiabChange)}</div></div></div>
  </div>`:''}
  <div class="section"><div class="sec-title">Suggested investment split</div>
    ...
    ${[['Flexi cap / Index fund','Long-term wealth creation','#4d9fff',sugFlexi,60],['Liquid assets / fund','Emergency & short-term','#a78bfa',sugLiquid,30],['Life & health insurance','Protection & risk cover','#00d4a0',sugIns,20]].map(([name,desc,color,amt,pct])=>`
    <div class="inv-row"><div><div class="ir-name">${name}</div><div class="ir-desc">${desc}</div></div><div><div class="ir-amt" style="color:${color}">${fmt(amt)}</div><div class="ir-pct">${pct}% of savings</div></div></div>`).join('')}
  </div>`;
  ...
  if(ctx){charts['income-split']=new Chart(ctx,{type:'doughnut',data:{labels:['Expenses','Invested','Surplus'],datasets:[{data:total>0?splitData:[1,1,1],backgroundColor:['#ff6b6b','#4d9fff','#00d4a0'],borderWidth:0,hoverOffset:6}]},options:{responsive:true,maintainAspectRatio:false,cutout:'68%',plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>total>0?' '+fmt(c.parsed)+' ('+Math.round(c.parsed/income*100)+'%)':' No data'}}}}});}
}
...
async function renderNetWorth(){
  ...
  ${monthlyNW.length>0?`<div class="section"><div class="sec-title">Net worth trend</div>
  ...
  <div class="section">
    <div class="sec-title"><span>Opening balances</span><button class="btn btn-green btn-sm" onclick="showOpeningForm()">+ Add balance</button></div>
    <p style="font-size:12px;color:#8892b0;margin-bottom:1rem;line-height:1.7">Add existing assets and liabilities you had before using this app.</p>
    ...
    </table></div>`:`<div class="empty" style="padding:1.5rem"><div class="empty-icon" style="font-size:24px">🏦</div>No opening balances added yet.</div>`}
  </div>`;
  if(monthlyNW.length>0){
    charts['nw-line']=new Chart($('nw-line-chart'),{type:'bar',data:{labels:monthlyNW.map(d=>MONTHS[d.m]+' '+String(d.y).slice(2)),datasets:[
    ...
    ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>' '+fmt(c.parsed)}}},scales:{x:{ticks:{color:'#8892b0',autoSkip:false,maxRotation:45},grid:{color:'rgba(99,120,220,0.08)'}},y:{ticks:{color:'#8892b0',callback:v=>fmt(v)},grid:{color:'rgba(99,120,220,0.08)'}}}}});
  }
}
...
async function calcMonthlyNetWorth(openingBals,allTxns){
  ...
  const minY=Math.min(...dates.map(d=>d.y)),maxY=Math.max(...dates.map(d=>d.y));
  const result=[];
...
window.showOpeningForm=function(){$('opening-form').style.display='block';$('ob-date').value=new Date().toISOString().split('T')[0];}
window.saveOpeningBal=async function(){
  const name=$('ob-name').value.trim(),cat=$('ob-cat').value;
  const subcat=$('ob-subcat').value,amount=fmtN($('ob-amount').value),date=$('ob-date').value;
  const msgEl=$('ob-msg');
  if(!name||!cat||!amount){setMsg(msgEl,'err','Please fill name, category and amount.');return;}
  setMsg(msgEl,'info','Saving…');
  await sb.from('opening_balances').insert({user_id:currentUser.id,name,category:cat,subcategory:subcat,amount,as_of_date:date});
  renderNetWorth();
}
window.deleteOB=async function(id){if(!confirm('Delete this opening balance?'))return;await sb.from('opening_balances').delete().eq('id',id);renderNetWorth();}

// ── TAB 4: ANNUAL ANALYSIS ─────────────────────
async function renderAnnual(){
  const body=$('pg-dash-body');const{y}=getMonthYear();
  ...
  const entries=[];
  for(let m=0;m<12;m++){
    const txns=allTxns.filter(t=>t.month===m);if(!txns.length)continue;
    let inc=0,exp=0,ast=0,lib=0;
    txns.forEach(t=>{const amt=parseFloat(t.amount);
      if(t.category==='Income')inc+=amt;if(t.category==='Expenses')exp+=amt;
  ...
  const sumInc=entries.reduce((a,e)=>a+e.inc,0),sumExp=entries.reduce((a,e)=>a+e.exp,0);
  const sumAst=entries.reduce((a,e)=>a+e.ast,0),sumSav=entries.reduce((a,e)=>a+e.savings,0);
  const avgRate=sumInc>0?Math.round(sumSav/sumInc*100):0;
  const bestM=entries.reduce((a,b)=>a.savings>b.savings?a:b);
  const annExpSubs={};
  allTxns.filter(t=>t.category==='Expenses').forEach(t=>{const sub=t.subcategory||'Other';annExpSubs[sub]=(annExpSubs[sub]||0)+parseFloat(t.amount);});
  const annExpArr=Object.entries(annExpSubs).sort((a,b)=>b[1]-a[1]);
  body.innerHTML=`
  <div class="section"><div class="sec-title">Saved months — ${y}</div>
    <div class="months-grid">
      ${entries.map(({m,inc,exp,ast,savings,count})=>`<div class="mcard" onclick="jumpMonth(${m})">
  ...
  <div class="section"><div class="sec-title">Year highlights — ${y}</div>
    <div class="hl-grid">
      ...
      <div class="hlcard"><div class="hl">Best month</div><div class="hv">${MONTHS[bestM.m]}</div><div class="hs">${fmt(bestM.savings)}</div></div>
    </div>
  </div>
  ...
  ${annExpArr.length?`<div class="section"><div class="sec-title">Annual expenses breakdown</div>
    ${annExpArr.map(([name,val])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:.5rem 0;border-bottom:1px solid rgba(99,120,220,0.08)">
      <span style="font-size:13px">${name}</span><div style="text-align:right"><div style="font-size:13px;font-weight:600">${fmt(val)}</div><div style="font-size:11px;color:#8892b0">${sumExp>0?Math.round(val/sumExp*100):0}%</div></div></div>`).join('')}
  </div>`:''}`;
...
window.jumpMonth=function(m){$('sel-month').value=m;switchTab('txn');}
...
async function renderRules(){
  ...
  <div class="section">
    ...
    <div class="field" style="max-width:240px;margin-bottom:1rem"><label>Your bank</label>
      <select id="rules-bank">${['kotak','hdfc','icici','sbi','axis','other'].map(b=>`<option value="${b}" ${(currentProfile.bank||'kotak')===b?'selected':''}>${b==='kotak'?'Kotak Mahindra':b==='hdfc'?'HDFC Bank':b==='icici'?'ICICI Bank':b==='sbi'?'State Bank (SBI)':b==='axis'?'Axis Bank':'Other Bank'}</option>`).join('')}</select>
    </div>
...
window.addRuleRow=function(kw='',cat='',subcat=''){
  const tbody=$('rules-body'),tr=document.createElement('tr');
  ...
  tr.innerHTML=`
    ...
    <td><select onchange="updateSubInRow(this)"><option value="">Select</option>${Object.keys(CAT_MAP).map(c=>`<option ${c===cat?'selected':''}>${c}</option>`).join('')}</select></td>
...
window.updateSubInRow=function(sel){
  const cat=sel.value,subSel=sel.closest('tr').cells[2].querySelector('select');
...
window.saveRules=async function(){
  ...
  const rules=rows.map(tr=>({keyword:tr.cells[0].querySelector('input').value.trim().toUpperCase(),cat:tr.cells[1].querySelector('select').value,subcat:tr.cells[2].querySelector('select').value})).filter(r=>r.keyword&&r.cat);
...
window.handlePDF=async function(e){
  ...
  const msgEl=$('upload-msg');setMsg(msgEl,'info','📖 Reading PDF… please wait.');
  try{
    ...
    for(let p=1;p<=pdf.numPages;p++){
      ...
      tc.items.forEach(it=>{const str=it.str.trim();if(!str)return;allItems.push({str,x:Math.round(it.transform[4]),y:Math.round(vp.height-it.transform[5])});});
    }
    const rules=await getRules(),txns=parseKotakPDF(allItems,rules);
    if(!txns.length){setMsg(msgEl,'err','No transactions found.');return;}
    setMsg(msgEl,'ok','✅ '+txns.length+' transactions found. Review below.');
    pendingTxns=txns;renderConfirmTable(txns);$('confirm-section').style.display='block';
...
function parseKotakPDF(items,rules){
  ...
  const lines=[];
  items.forEach(it=>{const line=lines.find(l=>Math.abs(l.y-it.y)<=6);if(line)line.items.push(it);else lines.push({y:it.y,items:[it]});});
  ...
  const inDesc=x=>x>=colX.desc-10&&x<colX.ref-5,inWd=x=>x>=colX.wd-10&&x<colX.dep-5,inDep=x=>x>=colX.dep-5&&x<colX.bal-5;
  const amtRe=/^[\d,]+\.\d{2}$/,dateRe=/^\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4}$/i,srRe=/^\d{1,3}$/;
  const mMap={jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};
  const isAmt=s=>amtRe.test(s.replace(/,/g,'')),toAmt=s=>parseFloat(s.replace(/,/g,''));
  ...
  lines.forEach(line=>{
    ...
    if(dateItem){
      ...
      const wdItem=line.items.find(it=>isAmt(it.str)&&inWd(it.x)),depItem=line.items.find(it=>isAmt(it.str)&&inDep(it.x));
  ...
  if(current&&current.desc.length>1&&current.amount>0)transactions.push(current);
  function fuzzyMatch(desc,keyword){const norm=s=>s.toUpperCase().replace(/[^A-Z0-9\s]/g,' ').replace(/\s+/g,' ').trim();return norm(desc).includes(norm(keyword));}
  const seen=new Set();
  return transactions.filter(t=>t.amount>0&&t.desc.length>1).map(t=>{
    const key=t.date+'|'+t.amount+'|'+t.type;if(seen.has(key))return null;seen.add(key);
    let cat='',subcat='';
...
window.updatePendingCat=function(i,cat){
...
window.confirmImport=async function(){
  ...
  const{error}=await sb.from('transactions').insert(toAdd.map(t=>({user_id:currentUser.id,date:t.date,description:t.desc,amount:t.amount,type:t.type,category:t.cat,subcategory:t.subcat,month:m,year:y})));
